...
const Dashboard = () => {
    ...
    useEffect(() => {
        if (!data) return
        setMints(data.TrendingMints.TrendingMint.map((mint: any) => `https://zora.co/collect/base:${mint.address}/${mint.erc1155TokenID ?? ''}`))
    }, [data])
    ...
    return (
        <div className="w-full min-h-screen pb-8">
            ...
            <div className="container max-w-5xl mx-auto flex flex-col gap-8">
                ...
                <div className="flex flex-col gap-1">
                    <h2 className="text-3xl font-semibold tracking-tight flex gap-2 items-center">
                        ...
                        Trending Mints on Base
                    </h2>
                    <p className="text-base opacity-80">Get them before it's too late!</p>
                    <ScrollArea className="w-full whitespace-nowrap pt-4">
                        <ul className="flex w-max gap-6">
                            {mints.map((frame) => (
                                <li key={frame} className="relative">
